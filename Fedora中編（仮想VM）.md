...
CPUの完全仮想化は(Intel:VT-x,AMD:SVM), dGPUは(Intel:VT-d,AMD:AMD-Vi, IOMMUの機能に相当します)というCPUのハードウェア支援機能によって行われています。

仮想化支援機能についてちょっと気になったので調べてみることに
### CPU仮想化支援機能とは
#### リングプロテクション・VT-x,AMD-V
...
リング3では特権命令を実行できないようにし、OSがリング0にいることによって、ユーザプロセスが特権命令や、メモリ・ハードウェアに不正アクセスしようとした際、制御権がリング0のOSに移るため、ユーザプロセスをOSが制御でき、ユーザプログラムが暴走しても、システムを保護することができます。
一方で、仮想化においては、ゲストOSをハイパーバイザが制御できるようにする必要があるため、リング0にハイパーバイザ、リング1にゲストOS、リング3にユーザプロセスがいるという構造になります。
この場合、ユーザプロセスが入出力をしようとした時に、スーパーバイザコールをしてOSに制御権を移すのと同様に、リング1にあるゲストOSが特権命令を実行しようとすると、トラップというものが発生し、ハイパーバイザに制御権が移ることで、ハイパーバイザがトラップの原因となった命令の処理を行い、命令実行が成功できたかのようにエミュレーションしてゲストOSを再び実行します。この仕組みをtrap-and-emulateと呼ぶそうです。
しかし、ここで問題が発生しました。
trap-and-emulateは、トラップが発生することを前提としていますが、本来リング0で動かすべきOSがリング1で動いている状態だと、コードセグメントレジスタという特権レベルを保持するレジスタの値が本来と異なるため、特権命令がトラップされないそうです。
また、非特権命令にも関わらず、ハードウェア状態を変更するような命令も存在するらしく、ハイパーバイザの関与なしにハードウェア状態を変更できてしまうことは仮想マシンの設計として望ましくないため、こちらも課題となったそうです。
このような命令群をセンシティブ命令と呼ぶそうです。
従来は、これらのセンシティブ命令を逐一他の命令へ書き換えるバイナリトランスレーションというのを行なっていたそう。
しかし、CPU仮想化支援機能によって、リング0をも支配できるモードが登場したことにより、そこにハイパーバイザが居座ることによって、ゲストOSがリング0で動作できるようになり、すべての命令がトラップできるようになりました。
